...
import {Direction, Orientation, Rect, Size} from '@react-types/shared';
...
export function useScrollView(props: ScrollViewProps, ref: RefObject<HTMLElement | null>): ScrollViewAria {
  ...
  let onScroll = useCallback((e) => {
    ...
    flushSync(() => {
      ...
      onVisibleRectChange({x: state.scrollLeft, y: state.scrollTop, width: state.width, height: state.height});
  ...
  let updateSize = useEffectEvent((flush: typeof flushSync) => {
    ...
    if (state.width !== w || state.height !== h) {
      ...
      state.height = h;
      flush(() => {
        onVisibleRectChange({x: state.scrollLeft, y: state.scrollTop, width: w, height: h});
      });
  ...
  useLayoutEffect(() => {
    if (!isUpdatingSize.current && (lastContentSize.current == null || contentSize.width !== lastContentSize.current.width || contentSize.height !== lastContentSize.current.height)) {
